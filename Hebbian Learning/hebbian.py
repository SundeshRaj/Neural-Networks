...
class Hebbian(object):
    ...
    def predict(self, X):
        ...
        if self.transfer_function == "Hard_limit":
            ...
            predictedValue = 1 / (1 + np.exp(-predictedValue))
    ...
    def train(self, X, y, batch_size=1,num_epochs=10,  alpha=0.1,gamma=0.9,learning="Delta"):
        ...
        #converting indexed target y to vectors
        ...
        for epoch in range(num_epochs):
            for i in range(0, X.shape[1], batch_size):
                batchLimit = i + batch_size
                if batchLimit > X.shape[1]:
                    batchLimit = X.shape[1]
                p = X[:,i:batchLimit]
                target = onehot[:,i:batchLimit]
                actual = np.dot(self.weights,p)
                if self.transfer_function == "Hard_limit":
                    actual[actual >= 0] = 1
                    actual[actual < 0 ] = 0
                elif self.transfer_function == "Sigmoid":
                    actual = 1 / (1 + np.exp(-actual))
                                
                if learning == "Delta":
                    self.weights += alpha*(np.dot((target-actual),p.T))
                elif learning == "Filtered":
                    self.weights = (1 - gamma)*(self.weights) + alpha*(np.dot(target,p.T))
                elif learning == "Unsupervised_hebb":
                    self.weights += alpha * np.dot(actual,p.T)
    ...
    def calculate_percent_error(self,X, y):
        ...
        percentError = 0
        maxOutput = np.argmax(predict, axis=0)
        for i in range(predict.shape[1]):
            if (maxOutput[i] != y[i]):
                percentError += 1
        return percentError/X.shape[1] 
    ...
    def calculate_confusion_matrix(self,X,y):
        ...
        """
        #initial confusion matrix with all zeros
        confusionMatrix = np.zeros((self.number_of_classes, self.number_of_classes))
